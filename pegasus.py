...
class VAE(nn.Module):
    def __init__(self, intermediate_size=256, hidden_size=40):
        ...
        self.fc4 = nn.Linear(intermediate_size, 4 * 4 * 128)
        self.batch7 = nn.BatchNorm1d(4*4*128)
    ...
    def encode(self, x):
        ...
        out = self.batch2(self.pool(F.relu(self.conv2(out))))
        out = self.batch3(self.pool(F.relu(self.conv3(out))))
        out = self.batch4(self.pool(F.relu(self.conv4(out))))
    ...
    def decode(self, z):
        ...
        out = self.batch8(F.relu(self.deconv1(self.up(out))))
        out = self.batch9(F.relu(self.deconv2(self.up(out))))
        out = self.batch10(F.relu(self.deconv3(self.up(out))))
